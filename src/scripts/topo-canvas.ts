...
export function initTopoCanvas() {
  ...
  function pointOnSide(side: number): Point {
    switch (side) {
      case 0:
        return { x: Math.random() * W, y: EDGE_OFFSET };
      case 1:
        return { x: W - EDGE_OFFSET, y: Math.random() * H };
      case 2:
        return { x: Math.random() * W, y: H - EDGE_OFFSET };
      case 3:
        return { x: EDGE_OFFSET, y: Math.random() * H };
    }
    return { x: 0, y: 0 };
  }
  ...
  function buildMap() {
    ...
    for (let i = 0; i < roadCount; i++) {
      const s1 = ~~(Math.random() * 4),
        s2 = (s1 + 1 + ~~(Math.random() * 2)) % 4;
      const a = pointOnSide(s1),
        b = pointOnSide(s2);
      roads.push({
        pts: createRoad(a.x, a.y, b.x, b.y, MAIN_ROAD_SWAY + Math.random() * MAIN_ROAD_SWAY_RANGE),
        w: MAIN_ROAD_WIDTH_BASE + Math.random() * MAIN_ROAD_WIDTH_RANGE,
        o: MAIN_ROAD_OPACITY_BASE + Math.random() * MAIN_ROAD_OPACITY_RANGE,
        la: 1,
      });
    }
    ...
    for (let i = 0; i < branchCount; i++) {
      ...
      const angle =
        sampleAngle(mr.pts, frac) +
        (BRANCH_ANGLE_BASE + Math.random() * BRANCH_ANGLE_RANGE) * (Math.random() < 0.5 ? 1 : -1);
      const len = BRANCH_LENGTH_BASE + Math.random() * BRANCH_LENGTH_RANGE;
      let ex = p.x + Math.cos(angle) * len,
        ey = p.y + Math.sin(angle) * len;
      ...
      roads.push({
        pts: createRoad(p.x, p.y, ex, ey, 20 + Math.random() * 45, 4 + ~~(Math.random() * 3)),
        w: 0.7 + Math.random() * 0.8,
        o: 0.055 + Math.random() * 0.04,
        la: 2,
      });
    }
    ...
    for (let i = 0; i < twig1Count; i++) {
      ...
      const angle =
        sampleAngle(sr.pts, frac) + (0.3 + Math.random() * 1.2) * (Math.random() < 0.5 ? 1 : -1);
      ...
      roads.push({
        pts: createRoad(p.x, p.y, ex, ey, 8 + Math.random() * 20, 3 + ~~(Math.random() * 2)),
    ...
    for (let i = 0; i < twig2Count; i++) {
      ...
      const mid = {
        x: (p.x + ex) / 2 + (Math.random() - 0.5) * 4,
        y: (p.y + ey) / 2 + (Math.random() - 0.5) * 4,
      };
      roads.push({
        pts: [{ x: p.x, y: p.y }, mid, { x: ex, y: ey }],
        w: 0.2 + Math.random() * 0.2,
        o: 0.02 + Math.random() * 0.018,
        la: 4,
      });
    }
    ...
    for (let gy = 0; gy < gridY; gy++)
      for (let gx = 0; gx < gridX; gx++) {
        ...
        for (let cc = 0; cc < numClusters; cc++) {
          let cx = gx * cellW + Math.random() * cellW,
            cy = gy * cellH + Math.random() * cellH;
          const snap = nearestRoad(cx, cy, BUILDING_SNAP_DIST);
          if (snap) {
            cx = snap.x + (Math.random() - 0.5) * 25;
            cy = snap.y + (Math.random() - 0.5) * 25;
          }
          ...
          for (let bb = 0; bb < numBuildings; bb++) {
            ...
            const pts = [
              ...
              {
                x: ux + cosA * bw - sinA * bh + jitter(),
                y: uy + sinA * bw + cosA * bh + jitter(),
              },
            ...
            if (r2 < 0.4) {
              ux += cosA * (bw + 1) + (Math.random() - 0.5) * 2;
              uy += sinA * (bw + 1) + (Math.random() - 0.5) * 2;
            } else if (r2 < 0.7) {
              ux += -sinA * (bh + 1) + (Math.random() - 0.5) * 2;
              uy += cosA * (bh + 1) + (Math.random() - 0.5) * 2;
            } else {
              ux += cosA * bw * 0.5 - sinA * (bh + 1) + (Math.random() - 0.5) * 3;
              uy += sinA * bw * 0.5 + cosA * (bh + 1) + (Math.random() - 0.5) * 3;
            }
          }
        }
      }
    ...
    for (let i = 0; i < buildings.length; i++) {
      ...
      for (let d = 0; d < numDust; d++) {
        ...
        dust.push({
          x: bld.cx + (Math.random() - 0.5) * spread * 2,
          y: bld.cy + (Math.random() - 0.5) * spread * 2,
          sz: 0.3 + Math.random() * 1.1,
          ph: Math.random() * Math.PI * 2,
          bs: 0.3 + Math.random() * 2.5,
          sh: 0.8 + Math.random() * 2,
          it: 0.4 + Math.random() * 0.6,
          lp: Math.random() * Math.PI * 2,
          ls: 0.03 + Math.random() * 0.12,
          cr: density > 0.87 && Math.random() < 0.1,
        });
      }
    }
    ...
    for (let i = 0; i < freeDust; i++) {
      dust.push({
        ...
        bs: 0.2 + Math.random(),
        sh: 1,
        it: 0.1 + Math.random() * 0.2,
        lp: Math.random() * Math.PI * 2,
        ls: 0.02 + Math.random() * 0.06,
        cr: false,
      });
    }
  }
  ...
  function spawnRipple(rx: number, ry: number) {
    ripples.push({
      ...
      y: ry,
      r: 0,
      mr: RIPPLE_RADIUS_BASE + Math.random() * RIPPLE_RADIUS_RANGE,
      sp: RIPPLE_SPEED_BASE + Math.random() * RIPPLE_SPEED_RANGE,
      op: RIPPLE_OPACITY_BASE + Math.random() * RIPPLE_OPACITY_RANGE,
    });
  }
  ...
  function draw(timestamp?: number) {
    ...
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    for (let la = 5; la >= 1; la--)
      for (let i = 0; i < roads.length; i++) {
        const r = roads[i];
        if (r.la !== la) continue;
        ctx.lineWidth = r.w;
        ctx.strokeStyle = ROAD_COLOR_BASE + r.o + ')';
        drawSpline(r.pts);
      }
    ...
    for (let i = 0; i < dust.length; i++) {
      ...
      ctx.fillStyle = 'rgba(' + DUST_COLOR + ',' + brightness + ')';
      ctx.fill();
      if (brightness > 0.5) {
        ctx.beginPath();
        ctx.arc(d.x, d.y, sz * 0.3, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(' + DUST_CORE_COLOR + ',' + brightness * 0.5 + ')';
        ctx.fill();
      }
    }
  }
  ...
  resize();
  draw();
  document.getElementById('heroPanel')!.addEventListener('click', () => {
    buildMap();
  });
}
