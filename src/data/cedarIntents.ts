...
export const INTENTS: CedarIntent[] = [
  ...
  },
  {
    ...
    answer: "We do not pretend input-output models are something they are not. The classic input-output core assumes fixed production coefficients, effectively elastic supply, and linearity, and on its own it is a short-run, static estimate rather than a general-equilibrium forecast. That core is the shared starting point for the whole field. Where Lumecon goes further is layering modern econometric and machine-learning methods and higher-frequency data on top, so the estimate can track a fast-moving economy better than a model frozen at 1970s input-output. Where the underlying assumptions still matter for your case, the study surfaces them so a reviewer can weigh them honestly.",
  },
  ...
  },
  {
    ...
    triggers: [
      'machine learning', 'use machine learning', 'do you use ml', 'modern methods', 'modern econometric', 'econometric methods', 'is this just input output', 'is it just input output', 'more than input output', 'more than io', 'is it just io', 'beyond input output', 'federated learning', 'what methods do you use', 'novel methods', 'do you use ai to model', 'is it just a multiplier model',
    ],
    answer: "Yes, and that is a real difference. The input-output core is the shared foundation for the whole field, and we start there on the standard public data so the numbers stay recognizable. On top of it we layer modern econometric and machine-learning methods, novel alternative and high-frequency data, and federated learning across studies, all calibrated and benchmarked as we built. The idea is to start where everyone starts, then extend in ways a model frozen at 1970s input-output cannot, because the economy moves fast. Where a method or assumption matters for your result, the study shows it.",
  },
  {
    id: 'guarantees',
    chip: null,
