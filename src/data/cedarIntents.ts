...
export const INTENTS: CedarIntent[] = [
  ...
  },
  {
    ...
    answer: "Yes, and that is a real difference. The input-output core is the shared foundation for the whole field, and we start there on the standard public data so the numbers stay recognizable. On top of it we layer what economics has learned since: behavioral and causal methods, spatial tools, higher-frequency data, and machine learning, with federated learning across studies, all calibrated and benchmarked as we built. The idea is to start where everyone starts, then reflect how economies actually work, instead of assuming perfectly rational actors in clean markets the way a model frozen at 1970s input-output does. Where a method or assumption matters for your result, the study shows it.",
