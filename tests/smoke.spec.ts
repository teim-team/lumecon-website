...
test('team page renders every roster portrait and column', async ({ page }) => {
  await page.goto('/team', { waitUntil: 'networkidle' });
  await expect(page).toHaveTitle(/Team \| Lumecon/);

  // The portraits are generated by scripts/team/headshots.mjs and
  // committed; a 404 on one would leave an empty circle rather than an
  // error, so check they actually decoded. Every portrait but the first
  // is lazy, so the page has to be scrolled before they are asked for.
  const portraits = page.locator('.roster__photo');
  ...
  await expect
    ...
    .toEqual([284, 284, 284, 284, 284, 144, 144, 144]);

  // Every person carries both columns. An entry with an empty education
  // or experience list is a data gap, not a layout choice.
  const rows = page.locator('.roster__row');
  await expect(rows).toHaveCount(8);
  for (const list of ['.roster__list--edu', '.roster__list']) {
    const counts = await rows.evaluateAll(
      (items, selector) => items.map((item) => item.querySelectorAll(`${selector} li`).length),
      list,
    );
    for (const count of counts) expect(count).toBeGreaterThan(0);
  }

  // The column labels are printed once per section and read as a header
  // for the columns beneath them, so exactly two sets should be visible.
  await expect(page.locator('.roster__label:visible')).toHaveCount(4);
});
