...
| 15  | Seed results preview must ship in the product                  | Backend/App                    | The site now names the free account Seed and states its results behavior: direct effects visible on the results page, indirect/induced/total and exports locked behind an in-context upgrade. teim-app must implement the gating server-side (tier `free` in tierCapabilities withholds the locked figures — never CSS-only blur). Spec: docs/seed-tier-spec.md. Until it ships, the free account's "full results unlock on a paid plan" behavior is unchanged, so the site slightly over-states Seed; close the gap from the app side quickly.                                                                                             |
