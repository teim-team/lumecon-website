...
| 2   | Unified terminology: analysis / project / study | Partial | Vocabulary standard recorded in this repository's AGENTS.md (user-facing "analysis"; backend object "project"; "study" reserved). **Corrected 2026-09-18:** the app's AGENTS.md now carries it, in a Vocabulary standard block under §2, and `cedar`'s new AGENTS.md and agent prompt carry it too. The app-wide copy sweep is still open, and is now deliberately **blocked on item 9 below**: the app's routes and files already say "analyses" (`APP_ANALYSES_PATH`, `src/pages/Analyses.jsx`) while the page renders `<h1>Studies</h1>`, but changing that heading on a page still branded "Tribal Economic Impact" finishes neither migration. Measured scope: 213 "study" and 77 "studies" occurrences in the app's `src/`, of which a large share are identifiers that must not be renamed. |
...
| 9   | TEIM naming migration                           | Partial, and larger than this row said | **Corrected 2026-09-18, and this is now the blocking item for P1 2.** Two separate leaks were still live on the app's main branch and are fixed: the intake panel read "you review every suggestion before TEIM runs", and the dev gallery's kicker read "TEIM DESIGN SYSTEM". The bigger problem this row understated: **"Tribal Economic Impact" is still the app's visible brand** — the side-rail wordmark and its aria-label, the public shell, the wizard cover, the Terms, Privacy and Methodology page titles, Cedar's own canned intake answers, the default `EMAIL_FROM` sender and the verification and password-reset subject lines, and the export filename prefix. That is the retired entry-point taxonomy this site dropped in favour of the Cedar family. It is not a copy sweep: item 14 below has counsel still claiming those names as marks, and the export filename is pinned by four backend assertions and appears in files customers already hold. Full inventory in the app's `AGENTS.md` §9. Storage keys (`teim:*`), session names and the `.teim-rd` class root are contracts and stay. |
